...
# --- 4. SYSTEM PROMPTS (RTL GENERATION PHASE) ---
...
# --- 5. LCEL CHAINS (THE EXPORTS) ---

# Spec Phase Chains
architect_prompt = ChatPromptTemplate.from_messages([("system", architect_system_prompt), ("user", "Design Requirement: {user_input}")])
critic_prompt = ChatPromptTemplate.from_messages([("system", critic_system_prompt), ("user", "Audit this RDS Draft:\n\n{draft}")])
master_prompt = ChatPromptTemplate.from_messages([("system", master_system_prompt), ("user", "ARCHITECT DRAFT: {draft}\n\nCRITIC AUDIT: {critique}\n\nHUMAN OVERRIDES: {human_input}")])
...
# RTL Phase Chains
rtl_architect_prompt = ChatPromptTemplate.from_messages([("system", rtl_architect_system_prompt), ("user", "Target Language: {language}\n\nRDS Document:\n{rds_content}")])
rtl_critic_prompt = ChatPromptTemplate.from_messages([("system", rtl_critic_system_prompt), ("user", "Review this {language} code:\n\n{code}")])
rtl_refiner_prompt = ChatPromptTemplate.from_messages([("system", rtl_refiner_system_prompt), ("user", "Original Code: {code}\nLint Report: {critique}\nHuman Feedback: {feedback}\nLanguage: {language}")])
...
def generate_initial_spec(user_input, rag_context):
    ...
    draft = architect_chain.invoke({"user_input": user_input, "rag_context": rag_context})
...
def finalize_spec(draft, critique, human_input):
    ...
    return master_chain.invoke({"draft": draft, "critique": critique, "human_input": human_input})
